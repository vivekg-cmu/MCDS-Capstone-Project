...
class InputFeatures(object):
    ...
    def __init__(self, guid, input_ids, input_mask, token_type_ids, label_id, concepts=None, concepts_mask=None,
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        try:
            inputs_a = tokenizer.encode_plus(example.text_a, add_special_tokens=True,
                                             max_length=max_length - len(inputs['input_ids']) + 1)
            ...
            tmp_a, tmp_b, _ = tokenizer.truncate_sequences(inputs_a['input_ids'][:-1],
                                                           pair_ids=inputs['input_ids'][1:-1],
                                                           num_tokens_to_remove=len(inputs_a['input_ids']) + len(
        ...
        if ex_index < 5:
            ...
            if example.concepts:
                ...
                logger.info("commonsense_masks_full: %s" % " ".join(
                    [' '.join([str(xx) for xx in x]) for x in concepts_mask_full]))
                logger.info("commonsense_masks: %s" % " ".join([str(x) for x in concepts_mask]))
                logger.info("commonsense tokens: %s" % " ".join(
                    [' '.join([str(xx) for xx in tokenizer.convert_ids_to_tokens(x)]) for x in concepts]))
        features[-1].append(
...
class CommonsenseqaProcessor(DataProcessor):
    def __init__(self, data_dir):
        ...
        for sid in range(2):
            with open(
                    [os.path.join(data_dir, "train_rand_split.jsonl"), os.path.join(data_dir, "dev_rand_split.jsonl")][
                        sid], "r") as f:
                data = []
                for line in f:
    ...
    def __init__(self, data_dir):
        ...
        for sid in range(2):
            with open([os.path.join(data_dir, "train.jsonl"), os.path.join(data_dir, "dev.jsonl")][sid], "r") as f:
                ...
                for i in range(len(data)):
                    d = ['Q: ' + data[i]['context'] + ' ' + data[i]["question"]]
                    for option in ['answerA', 'answerB', 'answerC']:
                        d += ['A: ' + data[i][option]]
                    d += [labels[i] - 1]
                    self.D[sid] += [d]
...
class SocialIqaInjProcessor(DataProcessor):
    def __init__(self, data_dir):
        self.D = [[], [], []]
        len_dict = Counter()
        for sid in range(2):
            with open([os.path.join(data_dir, "train.jsonl"), os.path.join(data_dir, "dev.jsonl")][sid], "r") as f:
                ...
                for i in range(len(data)):
                    d = ['Q: ' + data[i]['context'] + ' ' + data[i]["question"]]
                    for k, option in enumerate(['answerA', 'answerB', 'answerC']):
                        d += ['A: ' + data[i][option]]
...
class CommonsenseqaInjProcessor(DataProcessor):
    def __init__(self, data_dir):
        ...
        len_dict = Counter()
        for sid in range(2):
            with open([os.path.join(data_dir, "train_cs.jsonl"), os.path.join(data_dir, "dev_cs.jsonl")][sid],
                      "r") as f:
                data = []
                for line in f:
                    data.append(json.loads(line))
        ...
        with open(os.path.join(data_dir, "test_cs.jsonl"), 'r') as f:
            ...
            for i in range(len(data)):
                ...
                for k in range(5):
                    ...
                    examples.append(InputExample(guid=guid, text_a=question, text_b=candidate,
                                                 concepts=data[i]['choice_commonsense'][k]))
        return examples
