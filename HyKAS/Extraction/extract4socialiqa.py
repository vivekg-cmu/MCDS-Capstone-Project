...
def check_list(word, l,
               pos=None):  # this checks that word appear in context and its pos is in allowed list, if target concepts has pos, it needs to match the words' pos
    for w in l:
...
def check_valid(subj_anchor, subj, obj, obj_pos, target_seq, target_string, stopwords):
    ...
    if '_' in obj:
        if obj.replace('_', ' ') in target_string:
            ...
            matchs, target_word = get_matching_words(obj_tokens, target_seq, target_string, stopwords, subj,
                                                     subj_anchor)
...
def get_matching_words(obj_tokens, target_seq, target_string, stopwords, subj,
                       subj_anchor):  # just pick the first word for now
...
def build_trees(en_concetps, long_en_concepts, stopwords, question, options):
    ...
    for op in options:
                ...
                for phrase in long_en_concepts[word[0]]:
                    ...
                    if in_origin:
                        for rel in long_en_concepts[word[0]][phrase]:
                            in_context, match_obj = check_valid(word[0], phrase, rel[2], rel[3], option_pos, op,
                                                                stopwords)
                            if in_context:
        ...
        for word in option_pos:
            if word[0] not in stopwords and word[1][:2] in allowed_pos and word[0] in en_concepts:
                for rel in en_concepts[word[0]]:
                    in_context, match_obj = check_valid(word[0], word[0], rel[2], rel[3], question_pos, question,
                                                        stopwords)
                    if in_context:
                        addedge(graph, word[0], match_obj, word[0], rel[2], rel)
                ...
                for phrase in long_en_concepts[word]:
                    ...
                    if in_origin:
                        for rel in long_en_concepts[word][phrase]:
                            in_context, match_obj = check_valid(word, phrase, rel[2], rel[3], question_pos, question,
                                                                stopwords)
                            if in_context:
