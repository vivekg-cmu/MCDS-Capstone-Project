...
class QANet_Att_layer(nn.Module):
    def __init__(self, config):
        ...
        self.commonsense_encoder = nn.LSTM(input_size=config.hidden_size, hidden_size=100, batch_first=True,
    ...
    def forward(self, bert_output, commonsense, attention_mask, commonsense_mask, commonsense_shape):
        ...
        cs_attended = F.relu(self.att_proj(
            torch.cat([bert_output, weight_cs, bert_output * weight_cs, bert_output * weighted_context], dim=2)))
