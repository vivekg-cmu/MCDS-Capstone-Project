...
if __name__ == "__main__":
    ...
    with open(args.input_data, 'r') as f:
        for lines in f:
            raw_data.append(json.loads(lines))
        ...
        for sent in sents:
            ...
            if len(prefix_sent) > 17:
                ...
                for subsent in subsents:
                    ...
                    sent_atomic.append(outputs_subsent)
            else:
                outputs_sent = interactive.get_atomic_sequence(prefix_sent, model, sampler, data_loader, text_encoder, category)
                sent_atomic.append(outputs_sent)
        sample["context_atomic_sequence"] = sent_atomic

        question = sample["question"]
        #parsed_question = nlp(qa['question'])
        #pos = [w.tag_[:2] for w in parsed_question]
        #if 'VB' in pos:
        prefix_q, _ = data.atomic_data.do_example(text_encoder, question, None, True, None)
        subq_atomic = []
        if len(prefix_q) > 17:
            subqs = question.split(',')
            for subq in subqs:
                subq = subq.strip()
                ...
                subq_atomic.append(outputs_subq)
        else:
            outputs_q = interactive.get_atomic_sequence(prefix_q, model, sampler, data_loader, text_encoder, category)
            subq_atomic.append(outputs_q)
        sample["question_atomic_sequence"] = subq_atomic

        option_atomic = []
        for option in [sample["answerA"], sample["answerB"], sample["answerC"]]:
            o_atomic = []
            ...
            option_atomic.append(o_atomic)
        sample["option_atomic_sequences"] = option_atomic
        #pretty_print(sample)
        #exit(0)
